...
# Run aimpot2cube
python ../../aimpot2cube AIM.sh.o_spin_polarised.test test_spin_polarised.in.benchmark || { echo "Error: aimpot2cube failed to run" >&2; exit 3; }

test_spin_polarised="${test}-spin_polarised"

# Diff for result
diff_str=$(diff "AIM.sh.o_spin_polarised.test_eV.cube.benchmark" "AIM.sh.o_spin_polarised.test_eV.cube"); [ $? -gt 1 ] && exit 4
