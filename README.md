...
Scripts for setting up a regular grid of points for AIMPRO potential calculations and parsing the results to the .cube file format (potential values in eV)
...
If no input file is specified the default input file from common_io will be used. This file is user editable (see default_potfile_input/default.in).  

### Defining the potfile_gen input

It is often helpful to define grid with a reasonable dense sampling density within your system lattice.  
...
The sampling density therefor becomes the number of repeats, and by fixing the origin at (0.0 0.0 0.0) we obtain the contents of default_potfile_input/default.in:  
...
This script passes the output from an AIMPRO potential job (carried out with a pot_file generated by potfile_gen) into the cubefile (.cube) format (potential values in eV).  
